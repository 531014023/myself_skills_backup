...
> **核心原则**：用户资料和网络搜索是**同等重要**的两条腿，缺一不可。

### 4.1 资料的两条腿

| 来源 | 作用 | 使用时机 |
|------|------|---------|
| **用户提供的资料** | 深度细节 | 建立完善框架、刻画细节 |
| **网络搜索资料** | 广度 + 时效性 | 快速建立完整框架 |
...
| 类型 | 来源 | 处理 |
|------|------|------|
| ❌ 禁止使用 | 知乎、微信公众号、百度百科 | **严禁使用**，发现即删除 |
| ✅ 优先使用 | 伯克希尔官网、SEC文件（EDGAR） | 最高优先级 |
| ✅ 优先使用 | FT（金融时报）、WSJ、NYT | 权威媒体 |
| ✅ 优先使用 | Bloomberg、CNBC、福布斯 | 主流财经媒体 |
| ✅ 参考 | 财新（中文）、Farnam Street | 知识平台 |

#### 网络资料必须下载到本地

**所有网络搜索获取的资料必须下载保存到本地目录**：
...
**操作流程**：
1. 搜索并抓取网页内容（使用 web_fetch）
2. 保存为 Markdown 文件到 `references/web/` 目录
3. 文件命名格式：`序号-主题-日期.md`
4. 更新 `00-索引.md`，记录所有网络资料

### 4.3 用户资料的深度补充作用

- 用户资料提供**原始素材**（PDF原文、完整内容）
- 用于**深度细节**填充框架
- 用于**交叉验证**网络资料的准确性
...
- [ ] 网络搜索建立了完整框架（广度）？
- [ ] 用户资料填充了深度细节？
- [ ] 所有网络资料已下载到本地 `references/web/` 目录？
- [ ] 无黑名单来源（知乎/微信公众号/百度百科）？
- [ ] 每条关键信息标注了可追溯来源？
