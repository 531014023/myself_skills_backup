...
> **核心原则**：用户资料优先，网络搜索补充；用户无资料时完全依赖网络搜索。

### 4.1 资料收集策略

| 情况 | 策略 |
|------|------|
| **用户提供了资料** | 用户资料与网络搜索共同建立框架——用户资料管深度，网络搜索管广度和时效性以及交叉验证准确性，两者并重 |
| **用户未提供资料** | 完全依赖网络搜索，尽力获取一切可用信息，不设特定领域限制 |

### 4.2 网络资料必须下载到本地

**所有网络搜索获取的、有价值的资料都必须下载保存到本地目录**，不分领域：
...
**操作流程**：
1. 搜索并抓取网页内容（使用 smart-web-fetch 或 web_fetch）
2. 保存为 Markdown 文件到 `sources/web/` 目录
3. 文件命名格式：`序号-主题-日期.md`
4. 更新 `00-索引.md`，记录所有网络资料

### 4.3 用户资料的深度补充作用

- 用户资料提供**原始素材**（PDF原文、完整内容、内部资料）
- 用于**深度细节**填充框架
- 用于**交叉验证**网络资料的准确性
- 资料冲突判断原则：资料之间发生矛盾时，根据来源可靠性判断——一手资料 > 二手资料 > 推测；权威媒体 > 第三方媒体；本人亲笔 > 他人转述

### 4.4 调研完成标志

以下条件**必须同时满足**才算调研完成：

- [ ] 有用户资料时：用户资料已用于建立核心框架，网络资料已补充广度
- [ ] 无用户资料时：网络搜索已覆盖所有可用维度（著作、对话、表达、决策、时间线）
- [ ] 所有网络资料已下载到本地 `sources/web/`
- [ ] 无黑名单来源（知乎/微信公众号/百度百科）
- [ ] 信源可追溯（每条关键信息标注来源）
...
### 资料收集
- [ ] 有用户资料时：用户资料优先使用，网络搜索补充了广度？
- [ ] 无用户资料时：网络搜索覆盖了足够多的维度？
- [ ] 所有网络资料已下载到本地 `sources/web/` 目录？
- [ ] 无黑名单来源（知乎/微信公众号/百度百科）？
- [ ] 每条关键信息标注了可追溯来源？
